     // Base de datos de preguntas
        ...
        // API de base de datos (MongoDB vía backend Express)
        const apiDatabase = {
            async saveScore(playerData) {
                ...
                if (!response.ok || !payload.success) {
                    throw new Error(payload.message || 'No se pudo guardar el puntaje');
                }
            ...
            async getLeaderboard(limit = 10) {
                const response = await fetch(`/api/scores/leaderboard?limit=${encodeURIComponent(limit)}`);
                ...
                if (!response.ok || !payload.success) {
                    throw new Error(payload.message || 'No se pudo obtener el leaderboard');
                }
        ...
        async function endGame() {
            ...
            try {
                const result = await apiDatabase.saveScore({
                ...
                const savedId = result.id || result._id || 'N/A';
                dbStatus.className = 'db-status success';
                dbStatus.innerHTML = `✅ Puntaje guardado exitosamente en MongoDB<br><small>ID: ${savedId} | ${new Date().toLocaleString()}</small>`;
        ...
        async function updateLeaderboard() {
            ...
            container.innerHTML = '<div style="text-align: center; color: #999;">Cargando leaderboard...</div>';
            
            let leaderboard = [];
            try {
                leaderboard = await apiDatabase.getLeaderboard(10);
            } catch (_error) {
                container.innerHTML = '<div style="text-align: center; color: #dc3545;">No se pudo cargar el leaderboard</div>';
                return;
            }
